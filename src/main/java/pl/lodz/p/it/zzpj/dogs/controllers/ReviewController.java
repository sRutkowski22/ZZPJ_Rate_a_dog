...
public class ReviewController {
    ...
    public ReviewDto getReview(@PathVariable String url,
                               @PathVariable String username) throws ReviewException {
    ...
    public List<ReviewDto> getReviewsForUser(@PathVariable String username) {
        return reviewService.getAllReviewsForUser(username).stream()
                .map(ReviewMapper::mapToDto)
                .collect(Collectors.toList());
    }
    ...
    public List<ReviewDto> getAllReviewsForBreed(@PathVariable String breed) {
        return reviewService.getAllReviewsForBreed(breed).stream()
                .map(ReviewMapper::mapToDto)
                .collect(Collectors.toList());
    }
    ...
    public List<ReviewDto> getAllReviewsForUrl(@PathVariable String url) {
        return reviewService.getAllReviewsForUrl(url).stream()
                .map(ReviewMapper::mapToDto)
                .collect(Collectors.toList());
    }

    @GetMapping("/reviews/date/{firstDate}/{secondDate}")
    public List<ReviewDto> getAllReviewsBetweenDate(@PathVariable LocalDateTime firstDate,
                                                    @PathVariable LocalDateTime secondDate) {
        return reviewService.getAllReviewsBetweenDate(firstDate, secondDate).stream()
                .map(ReviewMapper::mapToDto)
                .collect(Collectors.toList());
    }
    ...
    public int getAverageRating(@RequestBody DogDto dogDto) {
        List<ReviewDto> reviews = reviewService.getAllReviewsForUrl(dogDto.getUrl()).stream()
                .map(ReviewMapper::mapToDto)
                .collect(Collectors.toList());
        if (reviews.size() == 0) {
            return 0;
